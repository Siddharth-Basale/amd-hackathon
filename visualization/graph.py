...
def load_knowledge_graph(graph_file: Path) -> nx.MultiDiGraph:
    """Load knowledge graph from JSON file."""
    if not graph_file.exists():
        logger.warning("Knowledge graph file not found: %s", graph_file)
    ...
    logger.info(
        "Loaded knowledge graph: %s nodes, %s edges",
        len(G.nodes),
        len(G.edges),
    )
    return G
...
def build_chunk_lookup(doc_graph: nx.DiGraph) -> Dict[str, str]:
    lookup: Dict[str, str] = {}
    for node_id, data in doc_graph.nodes(data=True):
        if data.get("type") == "chunk":
            chunk_id = data.get("chunk_id")
            if chunk_id is not None:
                lookup[str(chunk_id)] = node_id
    return lookup


def normalize_chunk_reference(ref: Optional[str]) -> Optional[str]:
    if not ref:
        return None
    if "::chunk::" in ref:
...
def visualize_interactive(
    ...
    if knowledge_graph and knowledge_graph.number_of_nodes() > 0:
        chunk_lookup = build_chunk_lookup(G)
        for node_id, node_data in knowledge_graph.nodes(data=True):
            if node_data.get("type") != "entity":
                continue
            kg_node_id = f"kg::{node_id}"
            label = node_data.get("name", node_id)
            description = node_data.get("description", "") or "Knowledge entity"
            aliases = node_data.get("aliases", [])
            title_parts = [description] if description else []
            if aliases:
                title_parts.append(f"Aliases: {', '.join(aliases)}")
            net.add_node(
                kg_node_id,
                label=label[:30],
                color="#ffb347",
        ...
        for source, target, edge_data in knowledge_graph.edges(data=True):
            source_data = knowledge_graph.nodes[source]
            target_data = knowledge_graph.nodes[target]
            if source_data.get("type") != "entity":
                continue
            relation = edge_data.get("relation", "relation")
            title_lines = [relation]
            evidence = edge_data.get("evidence")
            if evidence:
                if isinstance(evidence, list):
                    title_lines.extend(evidence[:2])
                else:
                    title_lines.append(str(evidence))

            source_id = f"kg::{source}"
            if target_data.get("type") == "entity":
                target_id = f"kg::{target}"
                net.add_edge(
                    source_id,
                    target_id,
                    color="#ff8800",
                ...
                chunk_ref = normalize_chunk_reference(target_data.get("chunk_id") or target)
                doc_node = chunk_lookup.get(chunk_ref or "")
                if doc_node:
                    net.add_edge(
                        source_id,
                        doc_node,
...
def visualize_directory(
    ...
    *,
    ...
    """
    Run all visualizations for a directory containing *_document_graph.json.
    Used by email pipeline after each email is processed.
    Returns True on success, False if no graph found.
    """
    graph_file = find_graph_file(base_path)
    if not graph_file or not graph_file.exists():
        if not quiet:
            logger.warning("No graph file found in %s, skipping visualization", base_path)
        return False

    G = load_graph(graph_file)
    knowledge_graph: Optional[nx.MultiDiGraph] = None
    default_kg = graph_file.parent / "knowledge" / f"{graph_file.stem.replace('_document_graph', '')}_knowledge_graph.json"
    if default_kg.exists():
        knowledge_graph = load_knowledge_graph(default_kg)
    ...
    if PYVIS_AVAILABLE:
        ...
        if knowledge_graph and knowledge_graph.number_of_nodes() > 0:
            ...
            visualize_interactive(nx.DiGraph(), knowledge_only_file, knowledge_graph=knowledge_graph)
...
Examples:
  ...
  python visualizeGraphE.py --graph-file path/to/graph.json
    ...
    parser.add_argument(
        "--knowledge-graph",
        type=str,
        help="Path to knowledge graph JSON (defaults to <base>/knowledge/<name>_knowledge_graph.json)",
    )
    ...
    if args.knowledge_graph:
        knowledge_graph = load_knowledge_graph(Path(args.knowledge_graph))
    else:
        default_kg = graph_file.parent / "knowledge" / f"{graph_file.stem.replace('_document_graph', '')}_knowledge_graph.json"
        if default_kg.exists():
            knowledge_graph = load_knowledge_graph(default_kg)
            logger.info("Loaded knowledge graph overlay from %s", default_kg)
    ...
    if PYVIS_AVAILABLE:
        ...
        if knowledge_graph and knowledge_graph.number_of_nodes() > 0:
            ...
            visualize_interactive(nx.DiGraph(), knowledge_only_file, knowledge_graph=knowledge_graph)
            print(f"✓ Knowledge graph visualization: {knowledge_only_file}")
